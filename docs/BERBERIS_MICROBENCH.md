...
`sp_region_chain` and `sp_region_chain_unmapped` bring the suite to 23 scenarios.
They split 64 guest instructions into sixteen separately compiled regions, use
real translation-cache dispatch for the first fifteen, and balance a fixed,
non-dereferenced SP. The second scenario disables register mapping. Final PC,
SP, and x0 checks verify completion, while generated bytes count all regions.
Use the same harness source for both versions. See
[the stack operands and memory-branch validation](BERBERIS_STACK_MEMORY.md) for measured
results and limits of this synthetic comparison.

The benchmark's native `RunBenchmark` function has explicit 64-byte alignment.
Use that same harness source for both builds and verify the actual timed-loop
placement when investigating small differences. Matching 64-byte offsets alone
does not establish identical cache or predictor behavior; retain raw paired
measurements and do not treat a layout adjustment as proof of a production gain.
